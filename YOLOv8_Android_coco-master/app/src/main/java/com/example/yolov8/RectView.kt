...
class RectView(context: Context, attributeSet: AttributeSet) : View(context, attributeSet) {
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        results?.forEach {
            ...
            if(target==classes[it.classIndex])
            {
                target_count=target_count+1
                if(target_count>=50) {
                    val intent = Intent(context, GameMainActivity::class.java)
                    intent.putExtra("isMatched", true)
                    (context as GameMainActivity).finish()
                }
            }
        }
        super.onDraw(canvas)
    ...
    fun sendIntentIfMatchedClass(classIndex: Int, targetClass: Int, context: Context) {
